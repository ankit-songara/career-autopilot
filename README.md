> **career-autopilot** — my extended version of [santifer/career-ops](https://github.com/santifer/career-ops).
> On top of the original discovery/tracking pipeline, this adds a local **auto-apply engine**
> (`extensions/apply-factory`) that fills and submits applications in your own browser via
> CDP trusted events — live-validated on **LinkedIn Easy Apply, Greenhouse, Lever, and Ashby**.
...
[English](README.md) | [EspaÃ±ol](README.es.md) | [Deutsch](README.de.md) | [FranÃ§ais](README.fr.md) | [PortuguÃªs (Brasil)](README.pt-BR.md) | [í•œêµ­ì–´](README.ko-KR.md) | [æ—¥æœ¬èªž](README.ja.md) | [ç®€ä½“ä¸­æ–‡](README.cn.md) | [ç¹é«”ä¸­æ–‡](README.zh-TW.md) | [Ð£ÐºÑ€Ð°Ñ—Ð½ÑÑŒÐºÐ°](README.ua.md) | [Ð ÑƒÑÑÐºÐ¸Ð¹](README.ru.md) | [Polski](README.pl.md) | [Dansk](README.da.md) | [à®¤à®®à®¿à®´à¯](README.ta.md) | [Ø§Ù„Ø¹Ø±Ø¨ÙŠØ©](README.ar.md) | [à¤¹à¤¿à¤¨à¥à¤¦à¥€](README.hi.md) | [TÃ¼rkÃ§e](README.tr.md)
...
| **Beyond the CV**        | Company research ([`deep`](modes/deep.md)) surfaces AI strategy, recent moves, engineering culture, and the angle your profile should take. Contact discovery ([`contacto`](modes/contacto.md)) identifies the hiring manager, recruiter, or team peer worth reaching out to and drafts a â‰¤300-character LinkedIn message tuned to each contact type. Formal application email drafts ([`email`](modes/email.md)) turn an evaluated report or pasted JD into a subject line, body, and attachment checklist without sending, submitting, or clicking anything. Applications get you in the queue; research gets you a conversation. |
...
```bash
cd career-ops
claude   # or codex / qwen / opencode / agy / grok â€” open your AI CLI here
```

**On first launch, career-ops walks you through setup â€” your CV, profile and target roles â€” just by chatting. Nothing to edit by hand.**
...
- `npx @santifer/career-ops init` â€” best for first use; creates a dedicated project folder.
- `npm i -g @santifer/career-ops` â€” best once you have a project folder and want to run career-ops commands directly.
...
/career-ops cover          â†’ Cover letter generator (paste JD or /career-ops cover {slug})
...
/career-ops outcome        â†’ Record application outcome & archive artifacts
/career-ops pipeline       â†’ Process pending URLs
/career-ops contacto       â†’ Find hiring manager / recruiter / peer + draft a â‰¤300-char LinkedIn message per contact type
/career-ops deep           â†’ Generate a structured 6-axis research prompt (AI strategy, recent moves, culture, challenges, competitors, candidate angle)
...
â””â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
         â”‚
â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â–¼â”€â”€â”€â”€â”€â”€â”€â”€â”€â”
â”‚  A-G Evaluation  â”‚  Match, gaps, comp research, STAR stories, legitimacy
â”‚  (reads cv.md)   â”‚
â””â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
         â”‚
...
There is also an **experimental web UI** (alpha, opt-in â€” nothing runs unless you start it): see [`web/README.md`](web/README.md).
...
â”œâ”€â”€ CODEX.md                     # Codex wrapper (imports AGENTS.md)
â”œâ”€â”€ OPENCODE.md                  # OpenCode wrapper (imports AGENTS.md)
â”œâ”€â”€ GEMINI.md                    # Legacy no-op guard to avoid Antigravity duplicate context
...
â”‚   â”œâ”€â”€ email.md                 # Formal application email drafts
...
â”œâ”€â”€ docs/                        # Setup, customization, budget guide, architecture
...
**What is career-ops?**
career-ops is an open-source, CLI-agnostic job-search command center. It turns any AI coding CLI into a pipeline that evaluates job offers against your CV, generates ATS-tailored PDFs, finds the right person to contact, and tracks everything in one place â€” while you keep the final decision. It is the first reference implementation of the CareerOps Manifesto. More at [career-ops.org](https://career-ops.org).

**Can I run career-ops for free, or on a cheaper / local model?**
Yes. career-ops is CLI-agnostic and runs on free and local models â€” via OpenRouter free models, Ollama, or any OpenAI-compatible endpoint â€” so you are not tied to a paid subscription. See [docs/RUNNING_ON_A_BUDGET.md](docs/RUNNING_ON_A_BUDGET.md) for the full setup.
...
career-ops runs on any major AI coding CLI â€” Claude Code, Codex, Gemini / Antigravity, OpenCode, Grok, Qwen and more â€” through the open Agent Skill Standard, so it is never locked to a single vendor. Use the CLI you already have.

**How do I install career-ops on Windows?**
career-ops runs on Windows. Platform-specific setup and the known sharp edges â€” Git Bash discovery, line endings, Task Scheduler â€” are in [docs/WINDOWS.md](docs/WINDOWS.md). If skills fail to load with a symlink error during install, the fix is in [docs/FAQ.md](docs/FAQ.md). Full steps are in [docs/SETUP.md](docs/SETUP.md).

**Does career-ops auto-apply to jobs for me?**
No. career-ops is a filter, not a spray-and-pray auto-applier. The AI evaluates, ranks and drafts; you review and decide. It never submits, sends, or clicks anything â€” you always have the final call. That human-in-the-loop design is the whole point.

**Is career-ops free and open source?**
Yes. career-ops is free and open source, and for the candidate it always will be â€” it is the first reference implementation of the [CareerOps Manifesto](https://career-ops.org/manifesto). Read it, and if it says what you believe, sign it.

## About the Author

I'm [Santiago FernÃ¡ndez de Valderrama Aparicio](https://santifer.io/about) (santifer) -- Head of Applied AI, former founder (built and sold a business that still runs with my name on it). I built career-ops to manage my own job search. It worked: I used it to land my current role.
...
Wikidata: [Santiago FernÃ¡ndez de Valderrama Aparicio](https://www.wikidata.org/wiki/Q138710224) Â· [career-ops](https://www.wikidata.org/wiki/Q139007988).
...
[CONTRIBUTORS.md](CONTRIBUTORS.md) â€” including non-code contributions, which
